# Blog: https://fronkonstin.com/2018/12/19/spinning-pins/
# Github: https://github.com/aschinchon/spinning-pins/blob/master/spinning_pins.R
...
# more patterns
n_points  <- 20
closeness <- golden_angle/n_points
speed     <- golden_angle/n_points
v_angles <- seq(0, 2*pi*golden_angle, length.out = n_points)
...
ggplot(df) +
  geom_spoke(aes(x=x, y=y, angle = angle), radius = 1, color = "lightblue") +
  geom_point(aes(x+cos(angle), y+sin(angle)), size=4, color = "darkblue") +
  theme_void() + 
  coord_fixed() +
